<template>
  <CockpitCard>
    ...
    <template #default>
      ...
      <div class="about-me-text">"{{ user.text }}"</div>
      <hr />
      <ul class="details">
        <li v-if="user.place"><v-icon icon="$place"></v-icon> {{ user.place }}</li>
        <li v-if="user.work"><v-icon icon="$working"></v-icon> {{ user.work }}</li>
        <li></li>
      </ul>
      <ul class="social">
...
// Dummy user. TODO replace by real data
const user: User = {
  name: 'John Doe',
